import { defaultEquivalentMethods, defaultHashFn, defaultVerifyUrlFn } from './defaults.js'
...
export const areEquivalent = async <T>(
  ...
  const [verify1, verify2] = await Promise.all([verifyUrlFn(url1), verifyUrlFn(url2)])
